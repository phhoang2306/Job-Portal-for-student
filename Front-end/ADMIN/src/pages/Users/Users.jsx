...
const Users = () => {
  ...
  const getUsers = async () => {
    try {
      const res = await axios.get(`${localUrl}/user-accounts?count_per_page=5&page=${page}&username=${searchValue}`, 
      {
        headers: 
        {
      ...
      setUsers(res.data.data.user_accounts.data);
      setLastPage(res.data.data.user_accounts.last_page);
    } catch (err) {
  ...
  const handleUnban = async (id) => {
    try {
      await axios.put(
        `${localUrl}/user-accounts/unban/${id}`,
        {},
        {
          headers: {
            'Content-Type': 'application/json',
  ...
  const handleLock = async (id) => {
    ...
    try {
      await axios.put(
        `${localUrl}/user-accounts/lock/${id}`,
        {
  ...
  const handleUnlock = async (id) => {
    try {
      await axios.put(
        `${localUrl}/user-accounts/unlock/${id}`,
        {},
        {
          headers: {
            'Content-Type': 'application/json',
  ...
  return (
    <div className="Userdatatable">
      <div className="UserdatatableTitle">Tài khoản người dùng</div>
      <input
      type="text"
